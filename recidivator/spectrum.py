...
####
## Modified copy of spectrum class from sncosmo
...
from sncosmo.utils import warn_once, integration_grid
from sncosmo.constants import SPECTRUM_BANDFLUX_SPACING, HC_ERG_AA
...
class Spectrum(object):
    ...
    def __init__(self, wave, flux, error=None,
        ...
        if error is not None:
            ...
            self._errortck = splrep(self.wave, self._error, k=1)
    ...
    def bandflux(self, band):
        ...
        ferr = splev(wave, self._errortck, ext=1)

        if self._error is not None:
            flux = np.sum(wave * trans * f) * dwave / HC_ERG_AA
            fluxerr = np.sqrt(np.sum((wave * trans)**2 * ferr**2)) * dwave / HC_ERG_AA
            return flux, fluxerr
        else:
