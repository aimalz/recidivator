...
def clean_env_curves(fname_env, ftype):
...
def do_functional_PCA(data: np.array, ndist: int,
    ...
    df = pd.DataFrame(comp_orig[:npcs])
    comp_orig_path = os.path.join('./',
                              str(npcs) +'PCs/' + 'G15' + '/comp_orig_' + str(npcs) + 'fPC.csv')
    df.to_csv(comp_orig_path, index=False)
...
def make_catalog_per_fname(f1, comp_orig, model, npcs, patch, ngroups):
    data = pd.read_csv(f1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modeldir',
                        default='/media/CRP6/Cosmology/workdir/emille/results_norm/',
                        help='Location of ML model for GAMA environment curve clutersting')
    parser.add_argument('--chunkdir_SLICS',
                        default='/media/CRP6/Cosmology/recidivator/SLICS/flexible_envirocurves/',
                        help='Location of Simulation Chunks')
    parser.add_argument('--chunkdir_GAMA',
                        default='/media/CRP6/Cosmology/recidivator/GAMA/flexible_envirocurves/',
                        help='Location of GAMA Chunks')
    parser.add_argument('--gen_summaries', dest='generate_fit_summaries',
                        default=False, action='store_true',
                        help='Generate 3D Gaussian Stan fits to GAMA curves')
    parser.add_argument('--do_class', dest='do_classification',
                        default=False, action='store_true',
    ...
    parser.add_argument('--max_dist', dest='max_dist',
                        default=2.5)
    parser.add_argument('--nonorm', dest='norm',
                        default=True, action='store_false')
    ...
    if opts.do_classification:
        env_path = os.path.join(opts.chunkdir_GAMA,
        ...
        classes, models = build_cluster_models(matrix=matrix, ngroups=opts.gnum,
                                               output_dir=os.path.join(opts.modeldir, str(opts.npcs) + 'PCs/' + opts.patch),
                                               save=opts.savefile)

    if opts.generate_fit_summaries:
        gama_classes_file = opts.modeldir + str(opts.npcs) + 'PCs/' + opts.patch + '/classes_' + str(opts.gnum).zfill(2) + '_classes.csv'

        data_env = pd.read_csv(gama_classes_file)

        for l in range(opts.gnum):
            create_fit_summaries(data_env, lbl=l, str_z='all', patch=opts.patch, gnum=opts.gnum,
                                 iter=int(opts.iter), chains=4, warmup=int(int(opts.iter)/4),
                                 savefiles=opts.savefile, outdir=opts.outdir)

    model_path = os.path.join(opts.modeldir,
                              str(opts.npcs) + 'PCs/' + opts.patch + '/model_' + str(opts.gnum).zfill(2) + '_groups.pkl')
    model = pd.read_pickle(model_path)

    los_chunks_path = os.path.join(opts.chunkdir_SLICS,
    ...
    comp_orig_path = os.path.join(opts.modeldir,
                                  str(opts.npcs) +'PCs/' + opts.patch + '/comp_orig_' + str(opts.npcs) + 'fPC.csv')

    comp_orig = pd.read_csv(comp_orig_path, header=None).values

    for i, f1 in enumerate(los_chunks):
        if i == 0:
            samp = make_catalog_per_fname(f1, comp_orig, model, opts.npcs, opts.patch, opts.gnum)
        else:
            samp1 = make_catalog_per_fname(f1, comp_orig, model, opts.npcs, opts.patch, opts.gnum)
            samp = pd.concat([samp, samp1])

    if opts.savefile:
        save_path = os.path.join(opts.outdir,
                                 'results_LOS42_' + opts.patch + '_' + str(opts.gnum).zfill(2) + 'groups_' + str(opts.npcs) +'PCs.csv')
